...
export default function VerifyEmailForm() {
    ...
    const { getEmailCodeMutaion } = useRegister();

    // Auto verify if token exists
    useEffect(() => {
        if (token) {
            getEmailCodeMutaion.mutate({ token });
        }
    }, [token, getEmailCodeMutaion]);

    const onSubmit: SubmitHandler<codeInput> = async (data) => {
        getEmailCodeMutaion.mutate({ code: data.code });
    };
